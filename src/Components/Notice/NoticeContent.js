...
const NoticeContent = (props) => {
  ...
  useEffect(() => {
    const DataApi = async () => {
      try {
        ...
        setLoading(true);
        const response = await request("get", `/notice/${ContentId}`, {}, "");
  ...
  useEffect(() => {
    const FileApi = async () => {
      try {
        const response = await fileRequest("get", `/notice/files/1`, {}, "");
      ...
      <S.Background>
        ...
        <S.NoticeContant key={contentData.id}>
          ...
          <S.NoticeHeader>
            ...
            <S.NoTitle>{contentData.title}</S.NoTitle>

            <S.NoDay>{createTime[0]}</S.NoDay>
          </S.NoticeHeader>

          <S.NoticeContain>{contentData.description}</S.NoticeContain>
          ...
          {contentData.fileName && (
            <S.NoticeFile>
              ...
              <S.FileTitle>
                <div onClick={FileDownload}>{contentData.fileName}</div>
              </S.FileTitle>
            </S.NoticeFile>
          )}
        </S.NoticeContant>
